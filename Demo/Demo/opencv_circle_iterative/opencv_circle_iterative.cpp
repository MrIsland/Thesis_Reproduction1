...
void opencv_circle_iterative::set_mat(Size bs) {
	this->src_count = 0;
	this->board_size = bs;
...
void opencv_circle_iterative::init() {
...
void opencv_circle_iterative::_FindCorner(bool flag = true) {
	...
	for (int i = 1; i <= ln; i++) {
		...
		if (!flag) {
			if (findCirclesGrid(this->gray_src[i], this->board_size, image_points_buf) == 0) {
...
vector<Point2f> opencv_circle_iterative::cornersort(vector<Point2f> tmp, vector<Point2f> res) {
	vector <Point2f> ans;
	int ln = (int)res.size();
	for (int i = 0; i < ln; i++) {
